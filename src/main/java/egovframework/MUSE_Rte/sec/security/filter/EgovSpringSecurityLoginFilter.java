package egovframework.MUSE_Rte.sec.security.filter;
			...
			if (isRemotelyAuthenticated != null && isRemotelyAuthenticated.equals("true")) {
				try {
					...
					if (loginVO != null && loginVO.getId() != null && !loginVO.getId().equals("")) {
						...
						//스프링 시큐리티 로그인
						//httpResponse.sendRedirect(httpRequest.getContextPath() + "/j_spring_security_check?j_username=" + loginVO.getUserSe() + loginVO.getId() + "&j_password=" + loginVO.getId());
						...
						springSecurity.doFilter(new RequestWrapperForSecurity(httpRequest, loginVO.getUserSe() + loginVO.getId(), loginVO.getId()), httpResponse, chain);
