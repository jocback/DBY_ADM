<%@ page contentType="text/html;charset=utf-8"%><%@ page import="com.dext5.DEXT5Handler" %><% 
	...
	DEXT5.SetRealPath("D:/usr/dby/upload/DEXT");
	DEXT5.SetTempRealPath("D:/usr/dby/upload/DEXT/temp");
	...
	String strSourceFile = DEXT5.LastSaveFile();
	if(!result.equals("")) {
		String[] spStr = result.split("dext5editordata/");
		//spStr[1] = spStr[1].substring(5);
		//String fileNm = spStr[1];
		//fileNm = fileNm.substring(5);
		if(spStr.length > 1) {
			//result = "/cmm/dext/imageSrcdd.do?"+strSourceFile.replace("/usr/dby/upload/DEXT\\2018\\","");
			//result = "/cmm/dext/imageSrc.do?"+spStr[1] + "^path=2018" + "&physical=" + strSourceFile.replace("D:/usr/dby/upload/DEXT\\2018\\","") + "&contentType=image/gif";
			String strPath = spStr[1].substring(0,4);
			result = "/cmm/dext/imageSrc.do?"+spStr[1] + "^&path=" + strPath + "&physical=" + strSourceFile.replace("D:/usr/dby/upload/DEXT\\"+strPath+"\\","");
			// spStr[1]은 width^height 입니다.
		}
		out.print(result);
	}
