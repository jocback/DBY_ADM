		...
		<tr id="fileObj<c:out value="${fileVO.atchFileId}"/><c:out value="${fileVO.fileSn}"/>">
			...
			<c:choose>
				<c:when test="${updateFlag=='Y'}">
					...
					<input type="text" name="fileCn" style="width:500px;" value="<c:out value='${fileVO.fileCn}'/>" />																
					</div>
					<c:if test="${delCheck eq 'delck'}">
					<div style="float:left;width:100%;margin-top:8px;">
					<input type="checkbox" value="Y" onClick="javascript:fnFileDelOp(this);"/><input type="hidden" name="fileDel"/>삭제
					<input type="hidden" name="fileSnum" value="${fileVO.fileSn}"/>
					<input type="file" name="file_1" style="width:300px;" id="egovComFileUploader"/>
					</div>
					</c:if>
