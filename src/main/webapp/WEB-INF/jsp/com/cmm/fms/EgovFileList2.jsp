		...
		<tr id="fileObj<c:out value="${fileVO.atchFileId}"/><c:out value="${fileVO.fileSn}"/>">
			...
			<c:choose>
				<c:when test="${updateFlag=='Y'}">
					...
					<img src="/cmm/fms/getImage.do?atchFileId=<c:out value='${fileVO.atchFileId}'/>&fileSn=0" style="max-height:300px;">
					</a>
					<c:if test="${delCheck eq 'delck'}">
					<p class="checkBox">
					<input type="checkbox" name="iFileDelCk" id="iFileDelCk" value="Y" onClick="javascript:fnIFileDelCk(this);"/>
					<label for="iFileDelCk"><span></span>삭제</label>
					</p>
					</c:if>
