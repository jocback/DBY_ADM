...
<%@include file="../include/header.jsp"%>
	...
	function fnActResultList(pageNo) {
		...
		document.frm.action = "/boffice/actStat/statMovingList.do";
		document.frm.submit();
	}
	$(document).on("click",".act_excel",function(){
		document.frm.action = "/boffice/actStat/statMovingExcel.do";
		document.frm.submit();
	});
	</script>
  					...
  					<tbody>
  						...
  						<tr>
			                <td class="list-no"><c:out value="${(paginationInfo.totalRecordCount - ((paginationInfo.currentPageNo - 1) * paginationInfo.recordCountPerPage)) - status.index}"/></td>
  							...
  							<td class="chkmoney"><c:out value="${result.bprice2}"/></td>
  							<td><c:out value="${result.status2}"/></td>
  							<td><c:if test="${result.lectureSno eq '0'}">종합</c:if><c:if test="${result.lectureSno ne '0'}"><c:out value="${result.tName}"/></c:if></td>
